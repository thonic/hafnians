...
**Module name:** :mod:`thewalrus_local.csamples`

.. currentmodule:: thewalrus_local.csamples
...
def rescale_adjacency_matrix_thermal(A, n_mean, check_positivity=True, check_symmetry=True, rtol=1e-05, atol=1e-08):
...
def rescale_adjacency_matrix(A, n_mean, scale, check_positivity=True, check_symmetry=True, rtol=1e-05, atol=1e-08):
    r"""Returns the scaling parameter by which the adjacency matrix A
    should be rescaled so that the Gaussian state that encodes it has
...
def generate_thermal_samples(ls, O, num_samples=1):
    ...
    rs = 0.5 * ls / (1 - ls)
    return [
        np.random.poisson(np.abs(O @ (np.random.normal(0, np.sqrt(rs)) + 1j * np.random.normal(0, np.sqrt(rs)))) ** 2)
