...
**Module name:** :mod:`thewalrus_local.symplectic`

.. currentmodule:: thewalrus_local.symplectic
...
def expand(S, modes, N):
    ...
    """
    M = S.shape[0] // 2
    S2 = np.identity(2 * N, dtype=S.dtype) if not issparse(S) else sparse_identity(2 * N, dtype=S.dtype, format="csr")

    if issparse(S) and isinstance(S, (coo_array, dia_array, bsr_array)):
        # cast to sparse matrix that supports slicing and indexing
        warnings.warn("Unsupported sparse matrix type, returning a Compressed Sparse Row (CSR) matrix.")
        S = csr_array(S)
