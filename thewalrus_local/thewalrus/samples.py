...
**Module name:** :mod:`thewalrus_local.samples`

.. currentmodule:: thewalrus_local.samples
...
from thewalrus_local.loop_hafnian_batch import loop_hafnian_batch
from thewalrus_local.loop_hafnian_batch_gamma import loop_hafnian_batch_gamma
from thewalrus_local.decompositions import williamson
...
def _hafnian_sample(args):
    ...
    while j < samples:
        result = generate_hafnian_sample(cov, mean=mean, hbar=hbar, cutoff=cutoff, max_photons=max_photons)
...
def torontonian_sample_graph(A, n_mean, samples=1, max_photons=30, fanout=10, cutoff=1, parallel=False):
...
def photon_number_sampler(probabilities, num_samples, out_of_bounds=False):
    ...
    if out_of_bounds is False:
        ...
        return [
            np.unravel_index(np.random.choice(vals, p=probabilities), [cutoff] * num_modes) for _ in range(num_samples)
        ]
