...
**Module name:** :mod:`thewalrus_local.decompositions`

.. currentmodule:: thewalrus_local.decompositions
...
from thewalrus_local.symplectic import sympmat
...
def blochmessiah(S):
    ...
    # Changing Basis
    R = (1 / np.sqrt(2)) * np.block([[np.eye(N // 2), 1j * np.eye(N // 2)], [np.eye(N // 2), -1j * np.eye(N // 2)]])
