...
import numba
import numpy as np
from thewalrus_local import charpoly
...
def f(A, n):  # pragma: no cover
    ...
    # code translated from thewalrus_local matlab script
    count = 0
...
def hafnian(
    ...
    if matshape[0] == 3 and loop:
        return A[0, 0] * A[1, 2] + A[1, 1] * A[0, 2] + A[2, 2] * A[0, 1] + A[0, 0] * A[1, 1] * A[2, 2]
        ...
        for D in ps:
            if lower_end + D not in loop_haf:
                loop_haf[lower_end + D] = sum(
                    A[i - 1, t - 1] * loop_haf[tuple(item for item in lower_end + D if item not in set((i, t)))]
                    for i in D
                )
