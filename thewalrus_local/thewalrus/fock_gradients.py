...
**Module name:** :mod:`thewalrus_local.fock_gradients`

.. currentmodule:: thewalrus_local.fock_gradients
    ...
    for m in range(cutoff):
        for n in range(cutoff - m):
            ...
            if 0 < p < cutoff:
                ...
                grad_phi[m, n, p, 0] = (1j * 0.5 * u * v - 1j * 0.5 * u) * sqrt[m] * sqrt[p] * T[m - 1, n, p - 1, 0] - (
                    0.5 * u + 0.5 * u * v
                ) * sqrt[n] * sqrt[p] * T[m, n - 1, p - 1, 0]
        ...
        for n in range(cutoff):
            for p in range(cutoff):
                ...
                if 0 < q < cutoff:
                    ...
                    grad_phi[m, n, p, q] = (1j * 0.5 * u * v - 1j * 0.5 * u) * sqrt[m] * sqrt[p] * T[
                        m - 1, n, p - 1, q
