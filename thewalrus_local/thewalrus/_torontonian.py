...
import numpy as np
import numba
from thewalrus_local.quantum.conversions import Qmat, reduced_gaussian
...
def recursiveLTor(L, modes, A, n, gammaL):  # pragma: no cover
    ...
    for i in range(start, n):
        ...
        tot += ((-1) ** len(nextModes)) * np.exp(0.5 * lc) / np.sqrt(det) + recursiveLTor(Ls, nextModes, Az, n, gammaX)
    return tot
...
def rec_ltorontonian(A, gamma):  # pragma: no cover
    ...
    lc = Ls.conj().T @ Ls
    return np.exp(0.5 * lc) / np.sqrt(det) + recursiveLTor(L, np.empty(0, dtype=np.int_), A, n, gamma)
