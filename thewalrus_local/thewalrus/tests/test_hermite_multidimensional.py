...
def test_hafnian_batched_zero_loops_no_edges():
    ...
    A = np.zeros([n_modes, n_modes], dtype=complex)
    n_photon = 5
    v1 = np.array([hafnian_repeated(A, q, loop=True) for q in product(np.arange(n_photon), repeat=n_modes)])
...
def test_grad_hermite_multidimensional_vs_finite_differences(tol, renorm):
    ...
    grad_C, grad_R, grad_y = grad_hermite_multidimensional(gate, R, y, C, renorm=renorm, dtype=np.complex128)
