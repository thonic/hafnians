...
from thewalrus_local.symplectic import (
    rotation,
    squeezing,
    interferometer,
    two_mode_squeezing,
...
from thewalrus_local.random import random_covariance, random_interferometer, random_symplectic
from thewalrus_local import threshold_detection_prob
from thewalrus_local.quantum.fock_tensors import _prefactor
from thewalrus_local.quantum.photon_number_distributions import _squeezed_state_distribution
from thewalrus_local.quantum.adjacency_matrices import _mean_clicks_adj
from thewalrus_local.quantum import (
    reduced_gaussian,
    Xmat,
    Qmat,
    Amat,
...
def test_state_vector_two_mode_squeezed():
    ...
    mu = np.zeros([4], dtype=np.complex)
    exact = np.array(
        [(np.exp(1j * i * phase) * (nbar / (1.0 + nbar)) ** (i / 2) / np.sqrt(1.0 + nbar)) for i in range(cutoff)]
    )
...
def test_state_vector_coherent():
    ...
    alpha = beta[0]
    exact = np.array(
        [(np.exp(-0.5 * np.abs(alpha) ** 2) * alpha**i / np.sqrt(np.math.factorial(i))) for i in range(cutoff)]
    )
...
def test_update_with_noise_coherent(num_modes, parallel):
    ...
    means_expected = real_to_complex_displacements(np.concatenate((beta_expected, beta_expected.conj())), hbar=hbar)
...
def test_n_body_marginals_mismatch_shape():
    ...
    mu = np.zeros([4])
    with pytest.raises(ValueError, match="The covariance matrix and vector of means have incompatible dimensions"):
...
def test_n_body_marginals_too_high_correlation():
    ...
    cov = squeezing([2 * r1, 0, 2 * r2])
    mu = np.zeros([6])
    with pytest.raises(ValueError, match="The order of the correlations is higher than the number of modes"):
...
def test_click_cumulants(disp_scale, hbar):
    ...
    expected = (
        ...
        - threshold_detection_prob(mu01, cov01, [1, 1], hbar=hbar) * threshold_detection_prob(mu2, cov2, [1], hbar=hbar)
        - threshold_detection_prob(mu02, cov02, [1, 1], hbar=hbar) * threshold_detection_prob(mu1, cov1, [1], hbar=hbar)
        - threshold_detection_prob(mu12, cov12, [1, 1], hbar=hbar) * threshold_detection_prob(mu0, cov0, [1], hbar=hbar)
        + 2
