...
from thewalrus_local.quantum import entanglement_entropy, log_negativity
...
def random_cov(num_modes=200, pure=False, nonclassical=True):
    ...
    _, cov = passive_transformation(np.zeros([len(cov_out)]), cov_out, np.sqrt(eta) * np.identity(len(cov_out) // 2))
...
def test_entanglement_entropy_two_modes(r, hbar):
    ...
    cov = (hbar / 2) * two_mode_squeezing(2 * r, 0)
    expected = (np.cosh(r) ** 2) * np.log(np.cosh(r) ** 2) - np.sinh(r) ** 2 * np.log(np.sinh(r) ** 2)
