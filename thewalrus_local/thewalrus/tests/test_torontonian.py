...
from thewalrus_local.quantum import density_matrix_element, reduced_gaussian, Qmat, Xmat, Amat
from thewalrus_local.random import random_covariance
...
from thewalrus_local._torontonian import numba_vac_prob
...
def test_torontonian_tmsv_complex():
    ...
    Omat = np.tanh(r) * np.array([[0, 0, 0, phase], [0, 0, phase, 0], [0, phasec, 0, 0], [phasec, 0, 0, 0]])
    tor_val = tor(Omat)
    assert np.allclose(tor_val.real, 1.0)
