...
from thewalrus_local.samples import (
...
from thewalrus_local.quantum import gen_Qmat_from_graph, density_matrix_element, probabilities
from thewalrus_local.symplectic import two_mode_squeezing
...
class TestHafnianSampling:
    ...
    def test_single_squeezed_state_hafnian(self):
        ...
        assert np.allclose(rel_freq, x2, atol=rel_tol / np.sqrt(n_samples), rtol=rel_tol / np.sqrt(n_samples))
    ...
    def test_two_mode_squeezed_state_hafnian(self):
        ...
        probs[-1] = 1.0 - np.sum(probs[0:-1])  # Coarse grain all the probabilities past the threshold

        assert np.allclose(rel_freq, probs, atol=rel_tol / np.sqrt(n_samples), rtol=rel_tol / np.sqrt(n_samples))
    ...
    def test_displaced_two_mode_squeezed_state_hafnian(self):
        ...
        probs = np.real_if_close(
            np.array(
                [[density_matrix_element(mean, sigma, [i, j], [i, j]) for i in range(n_cut)] for j in range(n_cut)]
            )
        )
        freq, _, _ = np.histogram2d(samples[:, 1], samples[:, 0], bins=np.arange(0, n_cut + 1))
        rel_freq = freq / n_samples

        assert np.allclose(rel_freq, probs, rtol=rel_tol / np.sqrt(n_samples), atol=rel_tol / np.sqrt(n_samples))
    ...
    def test_displaced_single_mode_state_hafnian(self, sample_func):
        ...
        probs = np.real_if_close(np.array([density_matrix_element(mean, sigma, [i], [i]) for i in range(n_cut)]))
    ...
    def test_multimode_vacuum_state_hafnian(self, sample_func):
        ...
        samples = sample_func(sigma, samples=n_samples)  # hafnian_sample_classical_state(sigma, samples=n_samples)
...
class TestTorontonianSampling:
    ...
    @pytest.mark.parametrize("sample_func", [torontonian_sample_state, torontonian_sample_classical_state])
