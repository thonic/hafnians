...
**Module name:** :mod:`thewalrus_local.loop_hafnian_batch_gamma`

.. currentmodule:: thewalrus_local.loop_hafnian_batch_gamma
...
from thewalrus_local._hafnian import (
    precompute_binoms,
    matched_reps,
    find_kept_edges,
    f_loop,
...
from thewalrus_local.loop_hafnian_batch import add_batch_edges_odd, add_batch_edges_even
...
def loop_hafnian_batch_gamma(A, D, fixed_reps, N_cutoff, glynn=True):
    ...
    if oddmode is None:
        ...
        return _calc_loop_hafnian_batch_gamma_even(Ax, Dx, fixed_m_reps, batch_max, odd_cutoff, glynn=glynn)
    edges = add_batch_edges_odd(fixed_edges, oddmode)
    Ax = Anz[np.ix_(edges, edges)].astype(np.complex128)
    Dx = Dnz[:, edges].astype(np.complex128)
    batch_max = (N_cutoff - 1) // 2
    even_cutoff = 1 - (N_cutoff % 2)
    return _calc_loop_hafnian_batch_gamma_odd(Ax, Dx, fixed_m_reps, batch_max, even_cutoff, glynn=glynn)
