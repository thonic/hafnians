#!/usr/bin/env python3
"""
Haar ensemble batch driver (J=4).

One PBS array task runs a batch of Haar seeds. For each seed, geometries
A → B → C are written as separate sparse JSON files.
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Haar ensemble batch (J=4)")
    ...
    parser.add_argument(
        ...
        help="optional single geometry; default = all in GEOMETRY_ORDER",
    )
    ...
    geom_list = (args.geometry,) if args.geometry else tuple(config.GEOMETRY_ORDER)
    ...
    runner = _load_runner()
    print(
        f"Ensemble batch J=4: n={len(indices)} geometries={geom_list} "
    ...
    for idx in indices:
        ...
        for g in geom_list:
            out = eco.realization_json_path(ens_dir, idx, geometry=g)
            if eco.is_valid_realization(out, expected_seed=seed, expected_geometry=g):
                print(f"[{idx:06d}/{g}] SKIP seed={seed}")
                skipped += 1
                continue
            print(f"[{idx:06d}/{g}] RUN  seed={seed} -> {out.name}")
            runner.run_geometry(
                g,
    ...
    print(f"Batch finished: ran={ran} skipped={skipped}")
