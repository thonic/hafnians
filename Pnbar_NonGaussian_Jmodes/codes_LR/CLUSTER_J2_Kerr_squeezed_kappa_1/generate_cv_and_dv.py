...
def reshape_mat(m, MK_dict):
    ...
    new_mat = np.identity(N, dtype=np.complex128)
...
def create_bs(U, n, m, N):
    if n == m:
        return False
    y = np.identity(N, dtype=np.complex128)
    y[n][n] = U[0][0]
    y[m][m] = U[1][1]
    y[n][m] = U[0][1]
    y[m][n] = U[1][0]
...
def generate_cv_and_dv(alpha, K, M, N, single_mode=False):
    ...
    if single_mode:
        ...
        # displacement vector
        dv = np.zeros(M, dtype=np.complex128)
        dv[0] = dv[0] + alpha[0]
    ...
    cm = np.identity(N, dtype=np.complex128)
    ...
    dv = np.zeros(M, dtype=np.complex128)
    dv[0] = dv[0] + alpha[0]
    bs1 = np.identity(M, dtype=np.complex128)
    for j in range(1, M):
        bs = create_bs(U, 0, j, M)
        bs1 = bs @ bs1
        dv = bs @ dv
...
def rearrange_cv_and_dv(cvs, dvs, count, size):
    """
    Stack j single-copy (Σ, D) for Pr(n̄) / driver_code mode order.

    After rearrange: (s₁, h₁, s₂, h₂, …, s₁*, h₁*, …) — interleaved by copy.
    Used by probability_zero_check.py when j > 1.
    """
    def _diag_block_mat(tables):
        blk_xsize = [tbl.shape[1] for tbl in tables]
        out_blocks = []
        for i, tbl in enumerate(tables):
            t_before = np.zeros((tbl.shape[0], sum(blk_xsize[:i])))
            t_after = np.zeros((tbl.shape[0], sum(blk_xsize[i + 1 :])))
            out_blocks.append(np.hstack([t_before, tbl, t_after]))
    ...
    for key in cvs.keys():
        ...
        dim = len(cv)
        n = int(dim / 2)
        cv1.append(cv[0:n, 0:n])
        cv2.append(cv[0:n, n:dim])
        cv3.append(cv[n:dim, 0:n])
        cv4.append(cv[n:dim, n:dim])
        dv1.append(dv[0:n])
        dv2.append(dv[n:dim])

    block_cv1 = _diag_block_mat(cv1)
    block_cv2 = _diag_block_mat(cv2)
    block_cv3 = _diag_block_mat(cv3)
    block_cv4 = _diag_block_mat(cv4)
