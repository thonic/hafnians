"""
J=4 P(n̄) — parallel The Walrus low-rank hafnian (ordinary, not loop), geometries A → B → C.

Same Fiurášek + geometry layout as cat/Kerr LR; engine is ``low_rank_hafnian``
on the Takagi factor of B (μ unused).
"""
...
def run_geometry(
    ...
    *,
    ...
    b_mat, mu, scale, MK_dict = build_B_mu_scale(
        states, J=J, apply_hadamard=config.APPLY_HADAMARD
    )
    G = takagi_factor_b_mat(b_mat)
    t_build = time.perf_counter() - t0
    print(
        f"  B shape = {b_mat.shape}, Takagi rank = {G.shape[1]}, "
        f"||μ|| = {np.linalg.norm(mu):.3e} (unused), build = {t_build:.3f} s",
        flush=True,
    )
    ...
    result = {
        ...
        "n_workers": n_workers,
        "apply_hadamard": config.APPLY_HADAMARD,
        "engine": ENGINE,
    ...
    lines = [
        ...
        f"takagi_rank={G.shape[1]}  workers={n_workers}  sinh(r)=1",
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Parallel J=4 standard-Gaussian low-rank P(n̄)"
    )
